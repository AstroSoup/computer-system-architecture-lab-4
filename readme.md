...
Доступ к памяти осуществляется через 2 канала. Один доступен только для записи и адресуется регистром `SHADOW_AR`. Другой доступен и для чтения и для записи и адресуется регистром `AR`.
...
Полученное время исполнения (доступ к памяти осуществляется за 1 такт)
```
Program executed after 0075 ticks.
```
...
- store2_word --- записать в память слово в ячейки начинающиеся по адресу в SHADOW_AR
- store2_byte --- записать в память байт по адресу в SHADOW_AR
...
- `double_arophmetics` --- сумма массива 32-битных чисел из входного файла в 64-битное число. 
